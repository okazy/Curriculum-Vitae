...
| 分類      | 詳細                                                                   |
|---------|----------------------------------------------------------------------|
| 脆弱性の特定・評価と対応  | 脆弱性診断 (ベンダー, AeyeScan, Invicti), EOL管理, 脆弱性の評価, 脆弱性の対応 |
| セキュア開発・設計支援  | セキュアコーディング, 設計レビュー, 脅威モデリング (STRIDE)        |
| 監視(SOC)、侵害検知  | XDR(Taegis), EDR(CrowdStrike), SaaSセキュリティ(MDA, GWS), WAF, 内部不正監視        |
| インシデント対応  | インシデント対応フローの整備, インシデント管理/トリアージ/対応支援        |
| ガバナンス  | ISMS認証取得/維持/運用支援, 生成AI導入支援, 対外発信支援, 業務リスク相談        |
...
#### サービスセキュリティの品質向上 (2023/04 - 現在)

- 脆弱性診断（手動・自動）の計画・導入・実施
- 開発チームと連携した脆弱性管理ライフサイクルの構築

#### コーポレートセキュリティの強化 (2023/04 - 現在)

- サイバーディフェンス
  - XDR基盤の導入・運用による脅威のリアルタイム検知
  - インシデントレスポンス体制の整備と実行
  - 内部不正の検知と対策
- セキュリティガバナンス
  - ISMS認証取得/維持/運用支援
  - 生成AIなどの新技術導入におけるリスクアセスメントとガイドラインの策定

職務: バックエンドエンジニア

#### サービスの保守運用改善 (2021/10 - 2023/03)
...
#### レガシーシステムのリプレイス (2021/10 - 2023/03)
